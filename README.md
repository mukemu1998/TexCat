...
当前开发分支增加了 `快速工具模式 / 工作流模式 Beta` 切换。工作流模式 Beta 目前支持资源池同步、步骤结构编辑、缩放/格式压缩/命名规则的参数面板、保存/载入工作流 JSON，以及输出文件名、路径和同名冲突预览，但还不执行完整流水线处理；实际生产仍使用快速工具模式。当前这轮工作流推进先排除 `PBR辅助转换`，PBR 继续保留在快速工具模式中，等后续单独优化算法和默认预设后再考虑重新接入工作流。

...
当前开发分支增加了 `快速工具模式 / 工作流模式 Beta` 切换。工作流模式 Beta 目前支持资源池同步、步骤结构编辑、图片裁切/缩放/格式压缩/命名规则的参数面板、保存/载入工作流 JSON、输出文件名/路径/同名冲突预览，并且已经可以真实执行第一条更完整的工作流闭环：`图片裁切 -> 缩放 -> 格式与压缩 -> 命名规则 -> 导出`。其中裁切步骤会复用图片裁切模块里已经框选好的裁切框、单图/多图同位置模式、命名模板和输出格式；法线/黑白调整、通道拆分、通道合并仍在后续阶段接入。实际生产仍建议优先使用已经成熟的快速工具模式。当前这轮工作流推进先排除 `PBR辅助转换`，PBR 继续保留在快速工具模式中，等后续单独优化算法和默认预设后再考虑重新接入工作流。
